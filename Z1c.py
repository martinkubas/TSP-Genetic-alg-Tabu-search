...
def printPath(traveler, cityCoords):
    for i in range(len(traveler.tour)):
        if i == len(traveler.tour) - 1:
            print(distTwoCities(cityCoords[traveler.tour[i]], cityCoords[traveler.tour[0]]), traveler.tour[i],
                  cityCoords[i], "->", traveler.tour[0], cityCoords[0])
        else:
            print(distTwoCities(cityCoords[traveler.tour[i]], cityCoords[traveler.tour[i + 1]]), traveler.tour[i],
                  cityCoords[i], "->", traveler.tour[i + 1], cityCoords[i + 1])
...
def fitnessProbabilities(population):
    ...
    popFitness = [maxDist - dist for dist in populationDistances]
    popFitnessSum = sum(popFitness)
    if popFitnessSum == 0:
        return [1.0 / len(popFitness)] * len(popFitness)
    probabilities = [fitness / popFitnessSum for fitness in popFitness]
...
def fitnessRoulete(population, genFitnessProbs):
    ...
    rand = random.random()
    boolProbability = fitnessProbsCumSum > rand
    boolProbability = np.array(boolProbability)

    return population[np.where(boolProbability)[0][0]]


def tournamentSelection(population, tournamentSize=5):
...
def mutate(traveler, mutRate):
    for swapped1 in range(len(traveler.tour)):
...
def geneticAlg(cities, popSize, generations, mutRate):
    ...
    for generation in range(generations):
        ...
        for i in range(0, popSize, 2):
            parent1 = parents[i]
            parent2 = parents[i + 1] if i + 1 < popSize else parents[0]
